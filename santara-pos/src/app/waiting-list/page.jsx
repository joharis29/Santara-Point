...
const ChangeEmailModal = dynamic(() => import('@/components/UserManagementModals').then(mod => mod.ChangeEmailModal), { ssr: false });
const ChangeWhatsappModal = dynamic(() => import('@/components/UserManagementModals').then(mod => mod.ChangeWhatsappModal), { ssr: false });
const ChangePasswordModal = dynamic(() => import('@/components/UserManagementModals').then(mod => mod.ChangePasswordModal), { ssr: false });
...
function WaitingListContent() {
    ...
    const selesaiList = transactions.filter(t => t.status === 'Selesai').slice(-10);
                        ...
                        <div className="flex-1 flex flex-col bg-slate-50 border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                {menungguList.length === 0 ? (
                                    ...
                                    menungguList.map(trx => <QueueCard key={trx.id} trx={trx} type="Menunggu" handleChangeStatus={handleChangeStatus} />)
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col bg-slate-50 border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-blue-50/30">
                                {diprosesList.length === 0 ? (
                                    ...
                                    diprosesList.map(trx => <QueueCard key={trx.id} trx={trx} type="Diproses" handleChangeStatus={handleChangeStatus} />)
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col bg-slate-50 border border-slate-200 rounded-3xl overflow-hidden shadow-sm opacity-80">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                {selesaiList.length === 0 ? (
                                    ...
                                    selesaiList.slice().reverse().map(trx => <QueueCard key={trx.id} trx={trx} type="Selesai" handleChangeStatus={handleChangeStatus} />)
                                )}
                            </div>
                        </div>
                    </div>
...
const QueueCard = React.memo(({ trx, type, handleChangeStatus }) => {
    return (
